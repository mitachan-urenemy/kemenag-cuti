...
<aside :class="sidebarOpen ? 'translate-x-0 w-86' : '-translate-x-full lg:translate-x-0 lg:w-24 w-86'"
    ...
    <div :class="sidebarOpen ? 'w-86' : 'w-86 lg:w-24'"
        ...
        <div class="flex items-center px-6 py-6 border-b border-white/10 shrink-0 h-[88px] box-border transition-all duration-300"
            ...
            <div class="bg-white/50 backdrop-blur-sm rounded-xl p-1 transition-all duration-300"
                ...
                <img src="{{ asset('images/logo-kemenag.webp') }}" class="w-12 h-12" alt="Logo Kemenag">
            </div>
        ...
        <!-- Navigation Links -->
        <nav
            class="flex-1 px-4 py-4 space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
            ...
            <a href="{{ route('dashboard') }}"
                ...
                <x-lucide-layout-dashboard
                    class="w-5 h-5 flex-shrink-0 transition-colors duration-300 {{ request()->routeIs('dashboard') ? 'text-green-primary' : 'text-white/80 group-hover:text-white' }}" />
                <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-x-2"
                    x-transition:enter-end="opacity-100 translate-x-0" class="ml-3 text-sm tracking-wide truncate">
                    Dashboard
                </span>
            </a>
            ...
            <a href="{{ route('surat-tugas.index') }}"
                ...
                :class="sidebarOpen ? 'justify-start' : 'justify-center px-0'">
                <x-lucide-briefcase
                    class="w-5 h-5 flex-shrink-0 transition-colors duration-300 {{ request()->routeIs('surat-tugas.*') ? 'text-green-primary' : 'text-white/80 group-hover:text-white' }}" />
                <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-x-2"
                    x-transition:enter-end="opacity-100 translate-x-0" class="ml-3 text-sm tracking-wide truncate">
                    Surat Tugas
            ...
            <a href="{{ route('surat-cuti.index') }}"
                ...
                :class="sidebarOpen ? 'justify-start' : 'justify-center px-0'">
                <x-lucide-file-text
                    class="w-5 h-5 flex-shrink-0 transition-colors duration-300 {{ request()->routeIs('surat-cuti.*') ? 'text-green-primary' : 'text-white/80 group-hover:text-white' }}" />
                <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-x-2"
                    x-transition:enter-end="opacity-100 translate-x-0" class="ml-3 text-sm tracking-wide truncate">
                    Surat Cuti
            ...
            <a href="{{ route('riwayat-surat') }}"
                ...
                :class="sidebarOpen ? 'justify-start' : 'justify-center px-0'">
                <x-lucide-history
                    class="w-5 h-5 flex-shrink-0 transition-colors duration-300 {{ request()->routeIs('riwayat-surat') ? 'text-green-primary' : 'text-white/80 group-hover:text-white' }}" />
                <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-x-2"
                    x-transition:enter-end="opacity-100 translate-x-0" class="ml-3 text-sm tracking-wide truncate">
                    Riwayat Surat
            ...
            @if (auth()->user()->role === 'admin')
                ...
                <a href="{{ route('manajemen-pegawai.index') }}"
                    class="relative flex items-center px-3.5 py-3 rounded-xl transition-all duration-300 group
                                                      {{ request()->routeIs('manajemen-pegawai.*') ? 'bg-white text-green-primary shadow-lg font-semibold' : 'text-white/90 hover:bg-white/10 hover:translate-x-1' }}"
                    :class="sidebarOpen ? 'justify-start' : 'justify-center px-0'">
                    <x-lucide-users
                        class="w-5 h-5 flex-shrink-0 transition-colors duration-300 {{ request()->routeIs('manajemen-pegawai.*') ? 'text-green-primary' : 'text-white/80 group-hover:text-white' }}" />
                    <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0 -translate-x-2"
                        x-transition:enter-end="opacity-100 translate-x-0" class="ml-3 text-sm tracking-wide truncate">
                        Manajemen Pegawai
